...
# Create publishers
targetPixel = rospy.Publisher('/color/white/xyPixels', Point32, queue_size=10)
targetSp = rospy.Publisher('/color/white/xyMeters', Point32, queue_size=10)
img_pub	 = 	rospy.Publisher('/color/white/processedImage', Image, queue_size=10)
...
def getColor():
    ...
    while not rospy.is_shutdown():
        ...
        _, mask = cv2.threshold(frameGRY,225,255,cv2.THRESH_BINARY)
		...
		# then update the list of tracked points
            cv2.circle(frameGRY, (int(x), int(y)), int(radius),(0, 255, 255), 2)
            cv2.circle(frameGRY, center, 5, (0, 0, 255), -1)
