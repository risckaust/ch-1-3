...
###### State Machine Class ######
# States: {Idle, Takeoff, ObjectSearch, Picking, GoToDrop, Dropping, GoHome, Land}
# Possible Signals for each state:
#	Takeoff:	{'Done', 'Running'}
#	ObjectSearch:	{'Done', 'Running'}
#	Picking:	{'Done', 'Running'}
...
class StateMachineC( object ):
	def __init__(self, ns):
		...
		self.DROP_COORDS	= np.array([23.3, 12.5])	# Lat/Lon of Drop Zone/box
		...
		# State Machine topic & its publisher (TODO: messages need to be created)
		self.state_topic	= StateMachine()
		self.state_pub		= rospy.Publisher(ns+'/state_machine/state', StateMachine, queue_size=10)	###### TODO:  Define the StateMachine message in the package ######
	...
	def execute_picking(self):
		...
		# set altitude
		self.altK.zSp= self.ZGROUND + rospy.get_param(self.namespace+'/autopilot/altStep')
		while not picked and not rospy.is_shutdown():
			...
			if objectFound:										# found an object
				...
				self.altK.zSp = self.altK.z + 0.1*abs(self.altK.z)				# increase altitude gradually, until an object is seen again
