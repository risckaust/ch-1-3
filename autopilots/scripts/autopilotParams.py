...
def setParams(ns):
    ...
    rospy.set_param(ns+'/autopilot/altStep',1.5)        # initial altitude step command
    rospy.set_param(ns+'/autopilot/camOffset',0.0)      # camera offset from ground

    # ROS parameters for kAltVel
    rospy.set_param(ns+'/kAltVel/gP',1.0)
    rospy.set_param(ns+'/kAltVel/gI',0.1)
    rospy.set_param(ns+'/kAltVel/vMaxU',1.0)
    rospy.set_param(ns+'/kAltVel/vMaxD',0.5)

    # ROS parameters for kBodVel
    rospy.set_param(ns+'/kBodVel/gP',1.0)
    rospy.set_param(ns+'/kBodVel/gI',0.1)
    rospy.set_param(ns+'/kBodVel/vMax',5.0)
    rospy.set_param(ns+'/kBodVel/gPyaw',0.5)           # yaw proportional gain
    rospy.set_param(ns+'/kBodVel/yawOff',0.05)         # error to turn off yaw control (m)
    ...
    rospy.set_param(ns+'/kBodVel/feedForward', True)   # use EKF and feedforward estimates
